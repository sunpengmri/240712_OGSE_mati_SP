...
project_dir = 'D:\02_MriDataSet\03_Diffusion\01_OGSE\00_Head\liulabin5\';
subject='liulabin5';
...
result_path=[project_dir '/' subject '/results/'];
mkdir(result_path)
...
% impulsed results
vin_nii=[project_dir '/' subject '/' 'results' '/' 'vin.nii.gz']; 
d_nii=[project_dir '/' subject '/' 'results' '/' 'd.nii.gz']; 
Dex_nii=[project_dir '/' subject '/' 'results' '/' 'Dex.nii.gz']; 



% mask
mask_whole_nii='mask_whole.nii.gz';
mask_tumor_nii='mask_tumor.nii.gz';
mask_other_nii='mask_tissue.nii.gz';

mask_whole_nii=[project_dir '/' subject '/'  mask_whole_nii]; 
mask_tumor_nii=[project_dir '/' subject '/'  mask_tumor_nii]; 
mask_other_nii=[project_dir '/' subject '/'  mask_other_nii]; 
...
if (nii_combined == 1)
    ...
    ogse_nii=[project_dir '/' subject '/' ogse_name]; 
    ...
    pgse_nii=[project_dir '/'  subject '/'  'pgse.nii.gz']; 
    ...
    % imshow(pgse_raw(:,:,5,1),[])
    ogsen1_nii=[project_dir '/' subject '/' 'ogsen1.nii.gz']; 
    ...
    ogsen2_nii=[project_dir '/' subject '/' 'ogsen2.nii.gz']; 
...
subplot(2,4,1)
imshow(PGSE_ADC_Map(:,:,slicei),[0,0.0025]);colormap jet;
title("PGSE")
colorbar
subplot(2,4,2)
imshow(OGSEN1_ADC_Map(:,:,slicei),[0,0.0025]);colormap jet;
title("OGSEN1")
colorbar
subplot(2,4,3)
imshow(OGSEN2_ADC_Map(:,:,slicei),[0,0.0025]);colormap jet;  
title("OGSEN2")
colorbar

colorbar
subplot(2,4,4)
imshow(delta_ADC_Map_N1(:,:,slicei),[0,100]);colormap jet;  
title("rADC1")
colorbar

subplot(2,4,5)
imshow(delta_ADC_Map_N2(:,:,slicei),[0,100]);colormap jet;  
title("rADC2")
colorbar
saveas(gcf, [result_path 'ADC.png'])

subplot(2,4,6)
imshow(vin_map(:,:,slicei),[0,0.5]);colormap jet;  
title("vin")
colorbar
saveas(gcf, [result_path 'ADC.png'])
...
cellularity_nii=[result_path '/' 'delta_ADC_N2']; 
